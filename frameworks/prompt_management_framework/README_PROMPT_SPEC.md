...
✅ **Prompt 組切換** ⭐：一鍵套用整套 prompt，支援多組命名管理與預覽比較
✅ **共用程式碼架構** ⭐：預覽與實際執行使用相同的程式碼，確保 100% 一致性
✅ **輸入驗證** ⭐：check_inputs() pre-flight check，在 API 呼叫前發現缺失輸入
✅ **安全啟動**：三種安全模式的啟動腳本
...
# 實作細節（給 AI 看的）
# ============================================================================
prompt_manager_class     # PromptManager 類別完整設計（含 Prompt Set 方法）
script_prompt_conventions # Prompt 寫法規範（很重要！）
input_validation         # ⭐ 輸入驗證（check_inputs pre-flight check）
prompt_set_ui            # ⭐ Prompt 組切換（總覽頁面一鍵套用整套 prompt）
...
│   ├── stage2_prompts.json
│   ├── baselines/                 # Baseline 基準版本
│   ├── sets/                      # Prompt 組
│   │   ├── baseline/              # 自動從 baselines/ 建立
│   │   └── <custom_name>/         # 自訂組（stage1.json ~ stage7.json）
│   ├── backups/                   # 自動備份
